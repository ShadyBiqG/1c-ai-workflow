...
function Test-PipelineScopePaths { param([Parameter(Mandatory=$true)][string[]]$Paths,[Parameter(Mandatory=$true)][string]$ProjectRoot)
    $root=[IO.Path]::GetFullPath($ProjectRoot); foreach($p in $Paths){$c=Get-PipelineCanonicalPath $p $root; if($c -ne $p.Replace('\','/')){throw "Scope path is not canonical: $p"}; $parts=$c -split '/'; $current=$root; if($parts.Count -gt 1){foreach($part in $parts[0..($parts.Count-2)]){$current=Join-Path $current $part;if(Test-Path -LiteralPath $current){$item=Get-Item -LiteralPath $current -Force;if($item.Attributes -band [IO.FileAttributes]::ReparsePoint){throw "Scope crosses reparse point: $p"}}}}}
    return $true
}
