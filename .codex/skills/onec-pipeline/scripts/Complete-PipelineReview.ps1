...
$pipelineEvent = if ($Verdict -eq 'approved') { 'review_approved' } else { 'review_changes_requested' }
$state = Set-PipelineEvent -State $state -Event $pipelineEvent -CorrelationId "${TaskId}:${pipelineEvent}:${reviewId}" -Configuration $configuration -EvidencePath $savedEvidencePath
