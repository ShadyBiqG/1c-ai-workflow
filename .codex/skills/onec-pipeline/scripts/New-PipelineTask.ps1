...
$tombstones = New-PipelineTombstoneInventory -Baseline $baseline -ProjectRoot $ProjectRoot

$taskId = New-PipelineTaskId
$tasksRoot=Join-Path $ProjectRoot '.pipeline\tasks';$staging=Join-Path $tasksRoot ('.staging-'+$taskId);$finalTaskDirectory=Join-Path $tasksRoot $taskId;$taskDirectory=$staging
try { [IO.Directory]::CreateDirectory($tasksRoot) | Out-Null;[IO.Directory]::CreateDirectory($staging) | Out-Null
...
Write-PipelineImmutableJson -Path (Join-Path $taskDirectory 'tombstones.json') -Value $tombstones | Out-Null
...
[IO.Directory]::Move($staging,$finalTaskDirectory)
$taskDirectory=$finalTaskDirectory
...
[pscustomobject][ordered]@{
    ...
    $resolvedTasksRoot=[IO.Path]::GetFullPath($tasksRoot).TrimEnd('\')+'\'
    if($resolvedStaging.StartsWith($resolvedTasksRoot,[StringComparison]::OrdinalIgnoreCase) -and (Test-Path -LiteralPath $resolvedStaging)){
